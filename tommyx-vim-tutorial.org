...
- =ci)= (*change inside "()"*): delete all content inside the current round bracket, and enter insert mode. For example, if you have ~a = (something else)~, and cursor is at ~else~, then press ~di)~ to get ~a = ()~.
- =yiw=: copy word
